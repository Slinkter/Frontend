...
const useGetFeedPosts = () => {
    ...
    useEffect(() => {
        const getFeedPosts = async () => {
            setIsLoading(true); // Marcar que se están cargando las publicaciones del feed
            ...
            // Consulta Firestore para obtener los posts de los usuarios seguidos por el usuario autenticado
            const q = query(
                collection(firestore, "posts"), // Referencia a la colección "posts" en Firestore
                where("createdBy", "in", authUser.following) // Filtrar los posts creados por usuarios en la lista de following del usuario autenticado
            );
    ...
    }, [authUser, showToast, setPosts, setUserProfile]); // Dependencias del efecto useEffect
