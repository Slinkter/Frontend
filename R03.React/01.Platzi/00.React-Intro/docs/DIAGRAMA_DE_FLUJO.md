...
sequenceDiagram
    ...
    useLocalStorage->>useLocalStorage: Establece isLoading=true
    main.jsx->>AppUI: Renderiza <AppUI>
    AppUI->>TodoProvider: Consume contexto (useContext)
    AppUI->>AppUI: Muestra "Cargando tus TODOs..." (isLoading es true)
    ...
    loop Simulación de Carga (1 seg)
        ...
        alt "V1" no existe
            ...
            useLocalStorage->>useLocalStorage: Parsea el JSON y actualiza el estado 'todos'
        end
        useLocalStorage->>useLocalStorage: Establece isLoading=false
    end

    TodoProvider-->>AppUI: El contexto se actualiza (isLoading: false, todos: [...])
...
2.  **`TodoProvider` se activa:**
    ...
    *   `useLocalStorage` establece su estado interno a `isLoading: true`.
3.  **Primer Render (`AppUI`):**
    *   `AppUI` consume el contexto (`useContext(TodoContext)`). En este momento, obtiene `isLoading: true`.
    *   El renderizado condicional muestra el mensaje "Cargando tus TODOs...".
4.  **Efecto de `useLocalStorage`:**
    ...
    *   Si no hay datos, guarda un array vacío `[]`. Si los hay, los parsea y los carga en su estado `todos`.
    *   Finalmente, actualiza su estado interno a `isLoading: false`.
5.  **Segundo Render (Re-renderizado):**
    ...
    *   `TodoProvider` ahora pasa los nuevos valores (`isLoading: false` y la lista de `todos`) a través del contexto.
    *   `AppUI`, al estar suscrito al contexto, detecta el cambio y se re-renderiza.
    *   Ahora `isLoading` es `false` y `searchedTodos` tiene datos, por lo que se muestra la lista de `TodoItem`.
...
sequenceDiagram
    ...
    CreateTodoButton->>AppUI: Llama a la función setIsModalOpen(true)
    AppUI-->>AppUI: Se re-renderiza porque el estado del contexto cambió
    AppUI->>Modal: Renderiza el Modal (porque isModalOpen es true)
    ...
    TodoProvider->>useLocalStorage: Llama a saveData(nuevoArray)
    TodoForm->>TodoProvider: Llama a setIsModalOpen(false) para cerrar el modal

    Note right of TodoProvider: El cambio de estado en 'todos' e 'isModalOpen' <br/> causa un re-renderizado.

    TodoProvider-->>AppUI: Provee los nuevos valores (lista actualizada e isModalOpen=false)
...
1.  **Abrir el Modal:**
    ...
    *   El botón llama a la función `setIsModalOpen(true)`, que fue obtenida del context.
    *   El estado `isModalOpen` en `TodoProvider` cambia a `true`.
    *   `AppUI` se re-renderiza y, como `isModalOpen` es `true`, ahora renderiza el componente `Modal`.
...
3.  **Actualizar el Estado:**
    *   La función `addTodo` en `TodoProvider` crea una copia del array de `todos`, añade el nuevo y llama a `saveData(nuevoArray)`.
    *   `saveData` (dentro de `useLocalStorage`) guarda el nuevo array en `localStorage` y actualiza el estado.
4.  **Cerrar el Modal y Re-renderizar:**
    *   Después de llamar a `addTodo`, la función `onSubmit` en `TodoForm` también llama a `setIsModalOpen(false)`.
    *   Los estados `todos` e `isModalOpen` han cambiado en `TodoProvider`.
...
sequenceDiagram
    ...
    TodoSearch->>TodoProvider: Llama a setSearchValue("nuevo valor") desde el contexto

    Note over TodoProvider: El estado 'searchValue' cambia. <br/> Se re-calcula el estado derivado 'searchedTodos'.
...
2.  **Evento `onChange`:**
    ...
    *   Este llama a `setSearchValue(e.target.value)`, función obtenida del `TodoContext`.
3.  **Actualización de Estado y Estado Derivado:**
    *   El estado `searchValue` dentro de `TodoProvider` se actualiza.
    *   `TodoProvider` se re-renderiza. En este nuevo render, la variable `searchedTodos` se **re-calcula**. Ya no es el array completo, sino el resultado de filtrar `todos` con el nuevo valor de `searchValue`.
...
sequenceDiagram
    ...
    TodoItem->>AppUI: Llama a la función completeTodo(text) (o deleteTodo)
    Note over AppUI: La función fue obtenida desde el contexto
...
2.  **Propagación del Evento (Hijo a Padre):**
    *   El `onClick` del `<span>` en `TodoItem` llama a la función que recibió como `prop` (`onComplete` o `onDelete`).
    *   Estas funciones fueron definidas en `AppUI` y llaman a las del contexto (`completeTodo` o `deleteTodo`).
3.  **Actualización del Estado Central:**
    *   La función `completeTodo` (o `deleteTodo`) en `TodoProvider` se ejecuta.
    *   Crea una copia del array de `todos`, busca el ítem por su `text`, lo modifica (o elimina), y llama a `saveData(nuevoArray)`.
4.  **Re-renderizado Final:**
    *   `saveData` actualiza el estado en `useLocalStorage`.
